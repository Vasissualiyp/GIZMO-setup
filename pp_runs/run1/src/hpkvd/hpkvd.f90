program hpkvdmain
  ...
  call mpi_init(ierr)                            ! from Intel mpi module on Niagara
  call mpi_comm_rank(mpi_comm_world,myid,ierr)   ! from Intel mpi module on Niagara
  call mpi_comm_size(mpi_comm_world,ntasks,ierr) ! from Intel mpi module on Niagara

  call hpkvd

  call mpi_barrier(mpi_comm_world,ierr)          ! from Intel mpi module on Niagara
...
subroutine hpkvd
  ...
  use textlib            ! from src/modules/External, procedures: i4arg,r4arg,r8arg,indlnb,getu,onedot,s2i4,s2r8
  ...
  character *512 rf_filepktab,rf_outcode,rf_incode,rf_fielddir
  ...
  ! I/O
  character*512           :: outputfilename, jboxout
  integer                 :: outnum
  ...
  call read_parameters ! reads hpkvd_params.txt (created by peak-patch.py from parameter file peak-patch/<path_to_run>/param/param.params)
  ...
  call RandomField_init(rf_filepktab,rf_ireadfield,rf_outcode,rf_incode,rf_fielddir,rf_boxsize,rf_nmesh,&
       rf_nbuff,rf_ntile,rf_ainit,NonGauss,fNL,A_nG,B_nG,R_nG,h) ! subroutine in src/modules/RandomField/RandomField.f90
  ...
  do i=1,nboxes       ! nboxes = total simulation tiles (i.e. for square volume nboxes=ntile^3)
     ...
     rbx = sqrt(xbx**2+ybx**2+zbx**2)   ! corner of tile i
  ...
  nboxes = m ! this is equivalent to no change nboxes = nboxes for non-lightcone runs
  ...
  do ibox=myid+1,ntasks*neach,ntasks
     ...
     findpeaks = .true.
     if(ibox+ntasks > ntasks*neach) report_sigma = .false. ! reduce in get_pks fails if every proccessor not involved
     if(ibox>nboxes) then
       jbox = boxlist(ibox-nboxes) ! periodic boundary conditions
       findpeaks = .false.
     else
       jbox = boxlist(ibox)
     endif

     xbx = xbox(jbox) ! (xbx,ybx,zbx) is the spatial coordinate within the simulation
     ybx = ybox(jbox) ! volume for a given lattice site (i.e. in Mpc), measured from
     zbx = zbox(jbox) ! the centre of the box for which (xbx,ybx,zbx)=(0,0,0).

     if(ievol==1) then ! for lightcone runs (redshift is a function of distance from centre of simulation box)
        rbx = sqrt(xbx**2+ybx**2+zbx**2) ! radial coordinate for (xbx,ybx,zbx)
...
subroutine read_parameters
  ...
  read(1,pos=1) ireadfield,ioutshear,&
       ...
       wsmooth,rmax2rs,ioutfield, NonGauss, fNL, A_nG, B_nG, R_nG,&
  ...
  if(verbose.and.myid==0) then
     write(*,*) ireadfield,ioutshear,&
          ...
          wsmooth,rmax2rs,ioutfield, NonGauss, fNL, A_nG, B_nG, R_nG,&
          ilpt, iwant_field_part, largerun
     write(*,'(a)') trim(fielddir)
     write(*,'(a)') trim(densfilein)
     write(*,'(a)') trim(filein)
     write(*,'(a)') trim(pkfile)
     write(*,'(a)') trim(filterfile)
     write(*,'(a)') trim(fileout)
     write(*,'(a)') TabInterpFile
endif
